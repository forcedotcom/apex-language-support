...
function copyWorkerFiles() {
  ...
  const workerSrc = path.resolve(__dirname, '../../apex-ls/dist/worker.mjs');
  const workerMapSrc = path.resolve(__dirname, '../../apex-ls/dist/worker.mjs.map');
...
function validateBuild() {
  ...
  const childProcessCount = (content.match(/require\("child_process"\)/g) || []).length;
  
  console.log(`📊 Node.js imports found: ${utilCount} util, ${urlCount} url, ${childProcessCount} child_process`);
  
  if (utilCount === 0 && urlCount === 0 && childProcessCount === 0) {
    console.log('✅ All Node.js imports properly polyfilled by tsup configuration');
  } else {
    console.log('ℹ️ Some Node.js imports present - these should be handled by tsup polyfill configuration');
  }
