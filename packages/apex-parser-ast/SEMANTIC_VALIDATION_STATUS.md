...
## Validation Deferral Snowflakes

These are spots in implemented validators that implicitly delegate final determination to org deployment—i.e., when the validator cannot resolve or verify something, it allows it through (or skips validation) rather than reporting an error. The org will accept or reject at deploy time.

### MethodResolutionValidator

| Location  | Pattern                                   | Deferral                                                                            |
| --------- | ----------------------------------------- | ----------------------------------------------------------------------------------- |
| 240–246   | Qualified call with unresolvable receiver | `continue` – skip validation rather than validating against containing class        |
| 808, 947  | Defer to symbol manager                   | Uses `isStandardLibraryType()` for built-in/standard types; otherwise no validation |
| 1067–1068 | Unknown visibility                        | `return true` – assume visible (conservative)                                       |
| 1334      | Not in assignment context                 | Skip return type checking                                                           |
| 1344      | Method unresolved or no return type       | Skip                                                                                |
| 1363–1365 | Couldn't determine variable type          | Skip                                                                                |
| 1367–1377 | Cross-file + qualified generic param      | Skip (e.g. `List<GeocodingService.Coordinates>`)                                    |
| 1860      | Unknown argument type                     | `return null` – treated as compatible                                               |
| 1889      | Unknown argument type                     | Fallback to Object when type cannot be determined                                   |

### DmlStatementValidator

| Location | Pattern                                              | Deferral                                                            |
| -------- | ---------------------------------------------------- | ------------------------------------------------------------------- |
| 180–181  | Method calls, complex expressions                    | Comment: "require TIER 2 resolution"                                |
| 205–206  | Unknown type in `List<X>`                            | `return true` – "could be SObject, variable name, etc."             |
| 279–281  | Method calls, complex expressions, unknown variables | `return true` – "We can't determine type without TIER 2 resolution" |
| 416–417  | Unknown in `isConcreteSObjectType`                   | `return true` – "could be concrete from method call etc."           |

### typeAssignability.ts

| Location | Pattern                         | Deferral                                                            |
| -------- | ------------------------------- | ------------------------------------------------------------------- |
| 63–64    | Empty source                    | `if (!source) return true` – "Unknown/fallback - skip strict check" |
| 51       | Empty target (method-parameter) | `return true`                                                       |

### InstanceofValidator

| Location | Pattern        | Deferral                                                          |
| -------- | -------------- | ----------------------------------------------------------------- |
| 320      | Left is Object | Skip INVALID_INSTANCEOF_ALWAYS_FALSE – "could be true at runtime" |

### TypeCastingValidator

| Location | Pattern           | Deferral                                                                    |
| -------- | ----------------- | --------------------------------------------------------------------------- |
| 165      | Object → SObject  | Comment: "would need runtime check"                                         |
| 170–172  | SObject → SObject | `return true` – "Simplified - in reality would check SObject compatibility" |
| 230–236  | Class hierarchy   | Hardcoded `ParentClass`/`ChildClass`; "would need actual inheritance info"  |

### VariableResolutionValidator

| Location     | Pattern                                                                 | Deferral                                                                                                                                 |
| ------------ | ----------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| 521–526      | objectName not from chain                                               | Fallback: `extractObjectNameFromFieldAccess` (source-text parsing) when chain doesn't provide base. Null objectName = listener gap.      |
| 594–596      | targetType unresolved                                                   | `targetType = containingClass` – when objectName/variable resolution fails, look up field on containing class; may report wrong type.     |
| 1188–1189    | Unknown visibility                                                      | `return true` – "assume visible (conservative)"                                                                                           |
| 1160–1161    | Can't determine declaring class                                         | `return true` – "assume visible (conservative)"                                                                                           |

When chain and source-text fallback both fail to provide objectName, field validation uses containingClass as target type and may report `FIELD_DOES_NOT_EXIST` on the wrong type. Org accepts valid code at deploy.

### StaticContextValidator

| Location | Pattern                                 | Deferral                                                 |
| -------- | --------------------------------------- | -------------------------------------------------------- |
| 362–365  | No resolution yet                       | "be conservative" – assume instance unless proven static |
| 408      | Fallback: check context if not resolved | Uses context heuristics                                  |

### TypeVisibilityValidator

| Location | Pattern                                 | Deferral                                        |
| -------- | --------------------------------------- | ----------------------------------------------- |
| 391      | Assume same-file if found in allSymbols | "For now, assume same-file"                     |
| 411      | Not same file                           | "Conservative: assume not visible"              |
| 414–415  | Unknown visibility                      | `return true` – "assume visible (conservative)" |

### UnknownAnnotationValidator

| Location | Pattern                     | Deferral                                                                                                 |
| -------- | --------------------------- | -------------------------------------------------------------------------------------------------------- |
| 39–42    | Doc comment                 | "does not determine if an annotation is valid for a specific org" – org-specific validity left to deploy |
| 138      | TIER 1 or resolution failed | Report as unknown; org may accept it                                                                     |

### ConstructorValidator

| Location | Pattern                      | Deferral                                                               |
| -------- | ---------------------------- | ---------------------------------------------------------------------- |
| 814      | Unknown type                 | "Return null to indicate unknown type (will be treated as compatible)" |
| 1088     | Parameter type not available | "assume mismatch to be safe" – opposite direction                      |

### ConstructorExpressionValidator

| Location | Pattern              | Deferral                                        |
| -------- | -------------------- | ----------------------------------------------- |
| 99–100   | Field type not found | "assume it's a String field for test purposes"  |
| 137      | SObject types        | "assume all SObject types have standard fields" |

### ExceptionValidator

| Location | Pattern      | Deferral                                                    |
| -------- | ------------ | ----------------------------------------------------------- |
| 578      | Conservative | "return null (conservative - don't report false positives)" |

### AnnotationPropertyValidator

| Location | Pattern              | Deferral                                            |
| -------- | -------------------- | --------------------------------------------------- |
| 595–596  | No version specified | "assume latest (allow all)"                         |
| 610      | No version specified | "assume latest (deprecated properties not allowed)" |

### Summary of Deferral Patterns

1. **Skip when unresolvable** – `continue` or early `return` when receiver/type cannot be resolved
2. **Source-text fallback when chain incomplete** – Parse source to extract object name when listener chain doesn't provide it; null objectName indicates listener gap. If both fail, targetType falls back to containingClass; org accepts valid code at deploy.
3. **Assume visible/compatible** – `return true` when visibility or compatibility unknown
4. **Unknown → Object** – Fallback to `Object` when type cannot be determined
5. **Unknown → null → compatible** – Return `null` for unknown type, treated as compatible
6. **Allow on uncertainty** – `return true` with comments like "could be SObject", "could be true at runtime"
7. **TIER 2 required** – Explicitly defer to TIER 2 or org for method calls and complex expressions
8. **Simplified / would need runtime** – Comments that real checks would need runtime or hierarchy info
9. **Conservative assumption** – Assume the permissive case (visible, compatible) when uncertain
...
2. **⚠️ Requires Schema/Metadata (10+ codes)**: Could potentially be implemented if schema/metadata is available, but currently stubbed due to missing metadata infrastructure:
   ...
   **Missing from stub classes** (requires Tooling API or metadata files):
   ...
   These differ from "Outside of Org" because they _could_ work with static metadata files or Tooling API access, but are not currently implemented due to missing infrastructure.
