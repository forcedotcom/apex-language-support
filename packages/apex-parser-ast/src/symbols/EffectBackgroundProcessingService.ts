...
/**
 * Effect-TS Queue Service for managing symbol processing tasks
 * This service manages the queue and delegates actual processing to ApexSymbolManager
 */
export class EffectSymbolQueueService {
...
/**
 * Main integration service that provides a simple interface for queuing symbol processing
 */
export class EffectBackgroundProcessingIntegration {
  private readonly queueService: EffectSymbolQueueService;
  private readonly logger = getLogger();

  constructor(symbolManager: ApexSymbolManager) {
    this.queueService = new EffectSymbolQueueService(symbolManager);
   ...
   */
  getTaskStatus(taskId: string): TaskStatus {
    return this.queueService.getTaskStatus(taskId);
  }
