...
- `src/semantics/`: Contains modules for performing semantic analysis and validation on the AST after it's built.
  - `validation/`: 2-tier semantic validation system:
    ...
    - `validators/`: 14 validator implementations (10 TIER 1, 4 TIER 2)
...
- **2-tier semantic validation system** with 14 validators
...
- **14 Validators**: 10 TIER 1 validators + 4 TIER 2 validators
...
1. **`ParameterLimitValidator`**: Validates method parameter count limits (max 32)
2. **`EnumLimitValidator`**: Validates enum constant count limits (max 100)
3. **`EnumConstantNamingValidator`**: Validates enum constant naming conventions
4. **`DuplicateMethodValidator`**: Detects duplicate method signatures (case-insensitive)
5. **`ConstructorNamingValidator`**: Validates constructor names match class names
6. **`TypeSelfReferenceValidator`**: Detects self-referencing type declarations
7. **`AbstractMethodBodyValidator`**: Validates abstract methods have no body
8. **`VariableShadowingValidator`**: Detects variable shadowing issues
9. **`ForwardReferenceValidator`**: Validates forward references
10. **`FinalAssignmentValidator`**: Validates final variable assignments
