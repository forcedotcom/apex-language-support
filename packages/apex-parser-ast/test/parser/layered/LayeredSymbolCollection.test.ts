...
describe('Layered Symbol Collection', () => {
  ...
  describe('BlockContentListener Detail Level', () => {
    it('should set detail level on local variable symbols via layered compilation', () => {
      const fileContent = `
        public class TestClass {
          public void publicMethod() {
            String localVar = 'test';
      ...
      const allSymbols = (publicResult.result as SymbolTable).getAllSymbols();

      // Find local variables (if captured at this layer)
      // Note: Local variables may be captured by BlockContentListener during processing
      const localVar = allSymbols.find(
        (s) => s.kind === SymbolKind.Variable && s.name === 'localVar',
      );
      const localInt = allSymbols.find(
        (s) => s.kind === SymbolKind.Variable && s.name === 'localInt',
      );
    ...
    it('should set detail level on semantic symbols from layered compilation', () => {
      const fileContent = `
        public class TestClass {
          public String publicField;
    ...
    it('should preserve detail level on symbols from ApexSymbolCollectorListener', () => {
      ...
      // Use layered listener which internally uses ApexSymbolCollectorListener
      // and now calls addSymbolWithDetailLevel
      const listener = new VisibilitySymbolListener('public-api', symbolTable);
      listener.setCurrentFileUri('TestClass.cls');
      const result = compilerService.compile(
        fileContent,
      ...
      const methodSymbol = semanticSymbols.find(
        (s) => s.kind === SymbolKind.Method && s.name === 'publicMethod',
      );
