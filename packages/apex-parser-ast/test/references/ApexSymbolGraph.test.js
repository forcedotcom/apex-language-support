...
const ApexSymbolGraph_1 = require("../../src/references/ApexSymbolGraph");
const symbol_1 = require("../../src/types/symbol");
describe('ApexSymbolGraph', () => {
    let graph;
    beforeEach(() => {
        graph = new ApexSymbolGraph_1.ApexSymbolGraph();
    });
    afterEach(() => {
        graph.clear();
    });
    // Helper function to create test symbols
    const createTestSymbol = (name, kind, fqn, filePath = 'TestFile.cls') => ({
        name,
        kind,
        ...
        modifiers: {
            visibility: symbol_1.SymbolVisibility.Public,
            isStatic: false,
            isFinal: false,
            isAbstract: false,
            isVirtual: false,
    ...
    describe('Symbol Management', () => {
        it('should add symbols to the graph', () => {
            const classSymbol = createTestSymbol('MyClass', symbol_1.SymbolKind.Class);
        ...
        it('should handle multiple symbols with the same name', () => {
            const class1 = createTestSymbol('MyClass', symbol_1.SymbolKind.Class, 'Namespace1.MyClass', 'File1.cls');
            const class2 = createTestSymbol('MyClass', symbol_1.SymbolKind.Class, 'Namespace2.MyClass', 'File2.cls');
        ...
        it('should lookup symbols by FQN', () => {
            const classSymbol = createTestSymbol('MyClass', symbol_1.SymbolKind.Class, 'MyNamespace.MyClass');
            graph.addSymbol(classSymbol, 'MyClass.cls');
        ...
        it('should get symbols in a file', () => {
            const classSymbol = createTestSymbol('MyClass', symbol_1.SymbolKind.Class);
            const methodSymbol = createTestSymbol('myMethod', symbol_1.SymbolKind.Method);
            graph.addSymbol(classSymbol, 'MyClass.cls');
        ...
        it('should get files containing a symbol', () => {
            const class1 = createTestSymbol('MyClass', symbol_1.SymbolKind.Class, 'MyClass', 'File1.cls');
            const class2 = createTestSymbol('MyClass', symbol_1.SymbolKind.Class, 'MyClass', 'File2.cls');
            graph.addSymbol(class1, 'File1.cls');
            graph.addSymbol(class2, 'File2.cls');
    ...
    describe('Reference Tracking', () => {
        it('should add references between symbols', () => {
            ...
            graph.addSymbol(classSymbol, 'MyClass.cls');
            graph.addSymbol(methodSymbol, 'MyClass.cls');
            graph.addReference(methodSymbol, classSymbol, ApexSymbolGraph_1.ReferenceType.METHOD_CALL, {
                startLine: 5,
                startColumn: 10,
                endLine: 5,
                endColumn: 20,
            ...
            expect(references[0].referenceType).toBe(ApexSymbolGraph_1.ReferenceType.METHOD_CALL);
        });
        it('should find references from a symbol', () => {
            const classSymbol = createTestSymbol('MyClass', symbol_1.SymbolKind.Class);
            const methodSymbol = createTestSymbol('myMethod', symbol_1.SymbolKind.Method);
            const fieldSymbol = createTestSymbol('myField', symbol_1.SymbolKind.Field);
            graph.addSymbol(classSymbol, 'MyClass.cls');
            graph.addSymbol(methodSymbol, 'MyClass.cls');
            ...
            graph.addReference(methodSymbol, fieldSymbol, ApexSymbolGraph_1.ReferenceType.FIELD_ACCESS, { startLine: 6, startColumn: 15, endLine: 6, endColumn: 25 });
        ...
        it('should handle deferred references for lazy loading', () => {
            ...
            const nonExistentSymbol = createTestSymbol('NonExistent', symbol_1.SymbolKind.Class);
            graph.addReference(methodSymbol, nonExistentSymbol, ApexSymbolGraph_1.ReferenceType.METHOD_CALL, { startLine: 5, startColumn: 10, endLine: 5, endColumn: 20 });
        ...
        it('should not create duplicate references', () => {
            ...
            graph.addReference(methodSymbol, classSymbol, ApexSymbolGraph_1.ReferenceType.METHOD_CALL, location);
            graph.addReference(methodSymbol, classSymbol, ApexSymbolGraph_1.ReferenceType.METHOD_CALL, location);
            const references = graph.findReferencesTo(classSymbol);
    ...
    describe('Dependency Analysis', () => {
        it('should analyze dependencies for a symbol', () => {
            const classA = createTestSymbol('ClassA', symbol_1.SymbolKind.Class, 'ClassA', 'ClassA.cls');
            const classB = createTestSymbol('ClassB', symbol_1.SymbolKind.Class, 'ClassB', 'ClassB.cls');
            const classC = createTestSymbol('ClassC', symbol_1.SymbolKind.Class, 'ClassC', 'ClassC.cls');
            ...
            graph.addReference(classA, classB, ApexSymbolGraph_1.ReferenceType.TYPE_REFERENCE, {
                startLine: 1,
                startColumn: 1,
                endLine: 1,
                endColumn: 10,
            });
            graph.addReference(classA, classC, ApexSymbolGraph_1.ReferenceType.TYPE_REFERENCE, {
                startLine: 2,
                startColumn: 1,
                endLine: 2,
                endColumn: 10,
            ...
            graph.addReference(classB, classC, ApexSymbolGraph_1.ReferenceType.TYPE_REFERENCE, {
                startLine: 1,
                startColumn: 1,
                endLine: 1,
                endColumn: 10,
        ...
        it('should calculate impact score correctly', () => {
            ...
            graph.addReference(classB, classA, ApexSymbolGraph_1.ReferenceType.TYPE_REFERENCE, {
                startLine: 1,
                startColumn: 1,
                endLine: 1,
                endColumn: 10,
            });
            graph.addReference(classC, classA, ApexSymbolGraph_1.ReferenceType.TYPE_REFERENCE, {
                startLine: 1,
                startColumn: 1,
                endLine: 1,
                endColumn: 10,
    ...
    describe('File Operations', () => {
        it('should remove all symbols from a file', () => {
            ...
            const otherSymbol = createTestSymbol('OtherClass', symbol_1.SymbolKind.Class, 'OtherClass', 'OtherFile.cls');
    ...
    describe('Edge Cases', () => {
        it('should handle symbols without FQN', () => {
            const symbol = createTestSymbol('MyClass', symbol_1.SymbolKind.Class);
    ...
    describe('Reference Types', () => {
        it('should handle different reference types', () => {
            ...
            graph.addReference(methodSymbol, fieldSymbol, ApexSymbolGraph_1.ReferenceType.FIELD_ACCESS, { startLine: 2, startColumn: 1, endLine: 2, endColumn: 10 });
            graph.addReference(classSymbol, methodSymbol, ApexSymbolGraph_1.ReferenceType.TYPE_REFERENCE, { startLine: 3, startColumn: 1, endLine: 3, endColumn: 10 });
            const references = graph.findReferencesFrom(methodSymbol);
            expect(references).toHaveLength(2);
            expect(references.map((r) => r.referenceType)).toContain(ApexSymbolGraph_1.ReferenceType.METHOD_CALL);
            expect(references.map((r) => r.referenceType)).toContain(ApexSymbolGraph_1.ReferenceType.FIELD_ACCESS);
        });
    });
});
//# sourceMappingURL=ApexSymbolGraph.test.js.map