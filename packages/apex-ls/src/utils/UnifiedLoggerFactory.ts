...
class UnifiedLogger implements LoggerInterface {
  ...
  public log(
    ...
    const message = `${timestamp} ${typePrefix} [${this.environment.toUpperCase()}] ${rawMessage}`;
    ...
    if (handler && typeof handler.sendLogMessage === 'function') {
      ...
      console.warn(
        '[UnifiedLogger] LogNotificationHandler not available or invalid. ' +
...
/**
 * A logger factory that creates instances of UnifiedLogger.
...
export class UnifiedLoggerFactory implements LoggerFactory {
  ...
  public getLogger(): LoggerInterface {
    if (!UnifiedLoggerFactory.loggerInstance) {
      UnifiedLoggerFactory.loggerInstance = new UnifiedLogger(this.environment);
    }
    return UnifiedLoggerFactory.loggerInstance;
  }
}
