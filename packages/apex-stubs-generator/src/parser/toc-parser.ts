...
const isCategoryNode = (node: any): boolean => {
  ...
  if (ownHref.includes("apex_class_") || ownHref.includes("apex_interface_")) return false;
  ...
  if (childHrefs.some(h => /_methods\.htm$|_constructors?\.htm$/.test(h))) return false;
...
const processClassNode = (node: any, nsName: string): ClassReference[] => {
  ...
  if ((pageType === "class" || pageType === "interface") && /\s/.test(className)) return [];
...
export const parseTocStructure = (tocJson: any) =>
  Effect.gen(function* () {
    ...
    for (const nsNode of root.children) {
      ...
      if (unique.length > 0) {
        namespaces.push({ name: nsName, pageId: nsHref.split("#")[0], classes: unique });
      }
    }
