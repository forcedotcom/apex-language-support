...
const program = Effect.gen(function* () {
  ...
  yield* Effect.tryPromise({
    try: () =>
      writeFile('doc-structure.json', JSON.stringify(structure, null, 2)),
    catch: (error) => new Error(`Failed to write structure: ${error}`),
  });

  yield* Console.log('Saved structure to doc-structure.json\n');

  const stringContent = yield* fetchPageContent(
    'apex_methods_system_string.htm',
  );

  yield* Effect.tryPromise({
    try: () =>
      writeFile('string-content.json', JSON.stringify(stringContent, null, 2)),
  ...
  if (structure.toc && structure.toc.length > 0) {
    ...
    yield* Console.log(
      `First entry: ${structure.toc[0].text || structure.toc[0].title || 'unknown'}`,
    );
  }
  ...
  if (stringContent.content) {
    ...
    yield* Console.log(
      `First 200 chars: ${stringContent.content.substring(0, 200)}`,
    );
  }
});
