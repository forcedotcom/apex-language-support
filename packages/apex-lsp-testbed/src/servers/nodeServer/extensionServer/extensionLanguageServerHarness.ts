...
export class ExtensionApexLanguageServerHarness {
  ...
  private findServerPath(): string {
    ...
    if (process.env.EXTENSION_LS_SERVER_PATH) {
      this.logger.info(`Using server path from environment: ${process.env.EXTENSION_LS_SERVER_PATH}`);
      return process.env.EXTENSION_LS_SERVER_PATH;
    }

    // Look for the built server module in the apex-ls-node package
    const extensionLsPackagePath = path.join(this.projectRoot, '..', 'apex-ls-node');
  ...
  async runTests(): Promise<void> {
    try {
      this.logger.info('Starting Extension Apex Language Server tests...');
 ...
 */
export function createExtensionLanguageServerHarness(): ExtensionApexLanguageServerHarness {
  return new ExtensionApexLanguageServerHarness();
}
