...
export interface ApexDocumentSymbolProvider {
   ...
   * @returns Effect that resolves to array of document symbols or symbol information
   */
  provideDocumentSymbols(
    params: DocumentSymbolParams,
  ): Effect.Effect<DocumentSymbol[] | SymbolInformation[] | null, never, never>;
}
  ...
  implements ApexDocumentSymbolProvider
{
  ...
  /**
   * Provides document symbols for the given document (Effect pattern)
   * @param params The document symbol parameters
   * @returns Effect that resolves to array of document symbols or symbol information
   */
  provideDocumentSymbols(
    ...
    return Effect.gen(function* () {
      ...
      try {
        ...
        const document = yield* Effect.promise(() =>
          self.storage.getDocument(documentUri),
        );
        logger.debug(
          () =>
            ...
            if (allWithSameId.length > 1) {
              ...
              for (const duplicateSymbol of allWithSameId) {
                if (
                  ...
                  const line =
                    duplicateSymbol.location.identifierRange.startLine;
                  locations.add(line);
        ...
        const symbolsResult = yield* self.provideDocumentSymbolsEffect(
          topLevelSymbols,
          symbolTable,
          logger,
        );
        ...
        for (const className of classNamesInResults) {
          ...
          for (const match of matches) {
            ...
            if (classKeywordIndex !== -1) {
              ...
              const afterClass = lineText
                .substring(classKeywordIndex + 5)
                .trim();
