...
const vsixFiles = fs.readdirSync(packageDir).filter(file => file.endsWith(`-${packageVersion}.vsix`));

if (vsixFiles.length === 0) {
  console.error(`No VSIX found in ${packageDir} matching version ${packageVersion}`);
  process.exit(1);
}
...
try {
  execSync(
    `npx vsce publish --pat ${vsceToken} --packagePath ${vsixPath} --skip-duplicate`,
    { stdio: 'inherit' }
  );
  console.log(`Successfully published ${vsixPath}`);
} catch (error) {
